  ...
  module MinimumMaximum
    ...
    private
      ...
      def validate(value, schema)
        ...
        # 3.1: exclusiveMinimum is a standalone numeric bound. We dispatch on
        # value type rather than the document's declared version so that a
        # writer who put a number here gets the strict-greater-than semantic
        # they intended even on a 3.0 document (SpecValidator reports the
        ...
        if schema.minimum
          # 3.0: exclusiveMinimum is a Boolean modifier on `minimum`. We
          # require an exact `== true` so a 3.1-style numeric does not
          # accidentally satisfy this branch via truthiness.
